...
const CONFIG = {
    ...
    AFFILIATE_CODE: "ufjv",
    PREDICTION_URL: "https://consultant-business-ia.free.nf/",
...
const TRANSLATIONS = {
    ...
    es: {
        flag: "🇪🇸",
        name: "Español",
        start_msg: "👋 ¡Bienvenido al Bot <b>DVYS</b>!\n\nÚnete a notre canal officiel pour commencer.",
        join_channel: "📢 Unirse",
        ...
        deposit_required: "💰 <b>Depósito Requerido</b>\n\nDeposita al moins <b>10 $</b> pour accéder aux signaux.",
    ...
    uz: {
        ...
        predictions_msg: "🔮 <b>Jonli Bashoratlar</b>\n\nDaromadingizni maksimal darajada oshirish pour jonli bashoratlarimizga kiring!",
...
async function supabaseInsert(table, data) {
    try {
        const response = await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/${table}`, {
            method: "POST",
            headers: { "Authorization": `Bearer ${CONFIG.SUPABASE_KEY}`, "apikey": CONFIG.SUPABASE_KEY, "Content-Type": "application/json", "Prefer": "return=representation" },
            body: JSON.stringify(data)
        });
        return response.json();
    } catch (e) { return null; }
...
async function getOrCreateUser(userId, userInfo = {}) {
    ...
    if (!users || users.length === 0) {
        const newUser = { user_id: userId, language: "fr", channel_joined: false, isregistered: "no", isdeposit: "no", created_at: new Date().toISOString(), updated_at: new Date().toISOString(), username: userInfo.username || null, first_name: userInfo.first_name || null, last_name: userInfo.last_name || null };
        await supabaseInsert("users", newUser);
        await supabaseInsert("user_sessions", { user_id: userId, current_menu: "start", session_data: {} });
        return newUser;
    }
    await supabaseUpdate("users", { last_activity: new Date().toISOString(), username: userInfo.username || users[0].username, first_name: userInfo.first_name || users[0].first_name, last_name: userInfo.last_name || users[0].last_name }, { user_id: userId });
    return users[0];
}

async function checkChannelMembership(userId) {
    try {
        const result = await telegramAPI("getChatMember", { chat_id: CONFIG.CHANNEL_ID, user_id: userId });
        if (result.ok) {
...
async function saveMessageHistory(userId, chatId, messageId, messageType = "photo", content = "") {
    const autoDeleteAt = CONFIG.AUTO_DELETE_MINUTES > 0 ? new Date(Date.now() + CONFIG.AUTO_DELETE_MINUTES * 60000).toISOString() : null;
    await supabaseInsert("message_history", { user_id: userId, chat_id: chatId, message_id: messageId, message_type: messageType, content: content, sent_at: new Date().toISOString(), auto_delete_at: autoDeleteAt, is_deleted: false });
}

async function deletePreviousMessages(chatId, userId) {
    try {
        const messages = await supabaseQuery("message_history", { user_id: userId, is_deleted: false });
        if (messages && messages.length > 0) {
            for (const msg of messages) {
                try {
                    await telegramAPI("deleteMessage", { chat_id: chatId, message_id: msg.message_id });
                    await supabaseUpdate("message_history", { is_deleted: true }, { id: msg.id });
                } catch (e) {}
            }
        }
    } catch (error) {}
...
async function sendPhotoWithButtons(chatId, photoUrl, caption, buttons, userId = null, deletePrevious = true) {
    if (deletePrevious && userId) await deletePreviousMessages(chatId, userId);
    const response = await telegramAPI("sendPhoto", { chat_id: chatId, photo: photoUrl, caption: caption, parse_mode: "HTML", reply_markup: { inline_keyboard: buttons } });
    if (userId && response.ok) await saveMessageHistory(userId, chatId, response.result.message_id, "photo", caption);
    return response.result?.message_id;
}

async function sendMessageWithButtons(chatId, text, buttons, userId = null, deletePrevious = true) {
    if (deletePrevious && userId) await deletePreviousMessages(chatId, userId);
    const response = await telegramAPI("sendMessage", { chat_id: chatId, text: text, parse_mode: "HTML", reply_markup: { inline_keyboard: buttons } });
    if (userId && response.ok) await saveMessageHistory(userId, chatId, response.result.message_id, "text", text);
    return response.result?.message_id;
}

function getLanguageKeyboard() {
    return [[{ text: "🇷🇺 Русский", callback_data: "lang_ru" }, { text: "🇬🇧 English", callback_data: "lang_en" }], [{ text: "🇮🇳 हिंदी", callback_data: "lang_hi" }, { text: "🇧🇷 Brazilian", callback_data: "lang_pt" }], [{ text: "🇪🇸 Español", callback_data: "lang_es" }, { text: "🇺🇿 O'zbek", callback_data: "lang_uz" }], [{ text: "🇦🇿 Azərbaycan", callback_data: "lang_az" }, { text: "🇹🇷 Türkçe", callback_data: "lang_tr" }], [{ text: "🇫🇷 Français", callback_data: "lang_fr" }, { text: "🇸🇦 العربية", callback_data: "lang_ar" }], [{ text: "← Back", callback_data: "back_menu" }]];
}

function getStartKeyboard(t) { return [[{ text: t.join_channel, url: CONFIG.CHANNEL_LINK }], [{ text: t.check_membership, callback_data: "check_membership" }]]; }
function getMainMenuKeyboard(t) { return [[{ text: t.btn_register, callback_data: "menu_register" }, { text: t.btn_instructions, callback_data: "menu_instructions" }], [{ text: t.btn_language, callback_data: "menu_language" }], [{ text: t.btn_get_signal, callback_data: "menu_get_signal" }]]; }
function getBackKeyboard(t) { return [[{ text: t.back, callback_data: "back_menu" }]]; }
...
async function handleUpdate(update) {
    try {
        if (update.message) {
            ...
            const user = await getOrCreateUser(userId, { username: update.message.from.username, first_name: update.message.from.first_name, last_name: update.message.from.last_name });
            const t = TRANSLATIONS[user.language || "fr"];
            if (text === "/start") await sendPhotoWithButtons(chatId, CONFIG.IMAGES.start, t.start_msg, getStartKeyboard(t), userId);
            else if (text === "/test") await telegramAPI("sendMessage", { chat_id: chatId, text: "✅ Le bot est en ligne sur Render !" });
        }
        if (update.callback_query) {
            ...
            const user = await getOrCreateUser(userId, { username: query.from.username, first_name: query.from.first_name, last_name: query.from.last_name });
            const t = TRANSLATIONS[user.language || "fr"];
            if (data.startsWith("lang_")) {
                const newLang = data.replace("lang_", "");
                if (TRANSLATIONS[newLang]) {
                    await supabaseUpdate("users", { language: newLang }, { user_id: userId });
                    const newT = TRANSLATIONS[newLang];
                    await sendPhotoWithButtons(chatId, CONFIG.IMAGES.menu, newT.welcome_back + "\n\n" + newT.main_menu, getMainMenuKeyboard(newT), userId);
                }
            } else if (data === "check_membership") {
                const isMember = await checkChannelMembership(userId);
                if (!isMember) { await telegramAPI("answerCallbackQuery", { callback_query_id: query.id, text: t.not_member, show_alert: true }); return; }
                await supabaseUpdate("users", { channel_joined: true }, { user_id: userId });
                await sendPhotoWithButtons(chatId, CONFIG.IMAGES.menu, t.main_menu, getMainMenuKeyboard(t), userId);
            } else if (data === "menu_register") {
                const regLink = `${CONFIG.ONE_WIN_DOMAIN}/?open=register&p=${CONFIG.AFFILIATE_CODE}&sub1=${userId}`;
                await sendPhotoWithButtons(chatId, CONFIG.IMAGES.register, t.register_msg, [[{ text: t.register_btn, url: regLink }], [{ text: t.back, callback_data: "back_menu" }]], userId);
            } else if (data === "menu_instructions") {
                await sendPhotoWithButtons(chatId, CONFIG.IMAGES.instructions, t.instructions_msg, getBackKeyboard(t), userId);
            } else if (data === "menu_get_signal") {
                if (user.isregistered !== "yes") {
                    const regLink = `${CONFIG.ONE_WIN_DOMAIN}/?open=register&p=${CONFIG.AFFILIATE_CODE}&sub1=${userId}`;
                    await sendPhotoWithButtons(chatId, CONFIG.IMAGES.signal, t.not_registered, [[{ text: t.register_btn, url: regLink }], [{ text: t.back, callback_data: "back_menu" }]], userId);
                } else if (user.isdeposit !== "yes") {
                    await sendPhotoWithButtons(chatId, CONFIG.IMAGES.deposit, t.deposit_required, getBackKeyboard(t), userId);
                } else {
                    await sendPhotoWithButtons(chatId, CONFIG.IMAGES.signal, t.access_granted, getBackKeyboard(t), userId);
                }
            } else if (data === "menu_language") {
                await sendMessageWithButtons(chatId, t.select_language, getLanguageKeyboard(), userId);
...
app.get('/', (req, res) => {
    res.send('🤖 Bot DVYS PRO est actif sur Render !');
});
